module YMatrix
    ...
    function getYsparse(factor::Bool=true)::SparseArrays.SparseMatrixCSC{Complex{Float64},Int}
        ...
        Utils.@checked Lib.YMatrix_GetCompressedYMatrix(factor ? 1 : 0, nBus, nNz, ColPtr, RowIdxPtr, cValsPtr)
    ...
    function VVector()
        ...
        Utils.@checked Lib.YMatrix_getVpointer(VvectorPtr)
    ...
    function getI()::Vector{ComplexF64}
        IvectorPtr = IVector()
        return unsafe_wrap(Array, IvectorPtr, Utils.@checked(Lib.Circuit_Get_NumNodes()) + 1)
    end

    """Get the data from the internal Voltage pointer"""
    function getV()::Vector{ComplexF64}
        VvectorPtr = VVector()
        return unsafe_wrap(Array, VvectorPtr, Utils.@checked(Lib.Circuit_Get_NumNodes()) + 1)
    end

    """Solve System for V"""
    function SolveSystem(NodeV::Vector{ComplexF64})::Int
        NodeV = Ref{Ptr{Cdouble}}(pointer(NodeV))
        return Utils.@checked Lib.YMatrix_SolveSystem(NodeV)
    end

    """SystemY has changed (Getter)"""
    function SystemYChanged()::Bool
        return Utils.@checked(Lib.YMatrix_Get_SystemYChanged()) != 0
    end

    """SystemY has changed (Setter)"""
    function SystemYChanged(Value::Bool)
        Utils.@checked Lib.YMatrix_Set_SystemYChanged(Value ? 1 : 0)
    end

    """Use auxiliary currents (Getter)"""
    function UseAuxCurrents()::Bool
        return Utils.@checked(Lib.YMatrix_Get_UseAuxCurrents()) != 0
    end

    """Use auxiliary currents (Setter)"""
    function UseAuxCurrents(Value::Bool)
        Utils.@checked Lib.YMatrix_Set_UseAuxCurrents(Value ? 1 : 0)
    end
