module Lines
    ...
    function New(Name::String)::Int
        return Utils.@checked Lib.Lines_New(Cstring(pointer(Name)))
    end
    ...
    function XMatrix(Value::Vector{Float64})
        Value, ValuePtr, ValueCount = Utils.prepare_float64_array(Value)
        Utils.@checked Lib.Lines_Set_Xmatrix(ValuePtr, ValueCount)
    end
    ...
    function Yprim(Value::Matrix{ComplexF64})
        Value, ValuePtr, ValueCount = Utils.prepare_float64_array(Value)
        Utils.@checked Lib.Lines_Set_Yprim(ValuePtr, ValueCount)
    end
