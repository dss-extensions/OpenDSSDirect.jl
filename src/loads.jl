module Loads
    ...
    function AllocationFactor()::Float64
        return Utils.@checked Lib.Loads_Get_AllocationFactor()
    end

    """Factor for allocating loads by connected xfkva (Setter)"""
    function AllocationFactor(Value::Float64)
        Utils.@checked Lib.Loads_Set_AllocationFactor(Value)
    end
    ...
    function PF(Value::Float64)
        Utils.@checked Lib.Loads_Set_PF(Value)
    end
    ...
    function PctMean()::Float64
        return Utils.@checked Lib.Loads_Get_PctMean()
    end
    ...
    function PctMean(Value::Float64)
        Utils.@checked Lib.Loads_Set_PctMean(Value)
    end
    ...
    function PctStdDev()::Float64
        return Utils.@checked Lib.Loads_Get_PctStdDev()
    end
    ...
    function ZipV(Value::Vector{Float64})
        ...
        Utils.@checked Lib.Loads_Set_ZIPV(ValuePtr, ValueCount)
    end
    ...
    function Daily(Value::String)
        Utils.@checked Lib.Loads_Set_daily(Cstring(pointer(Value)))
    end

    """Name of the loadshape for a duty cycle simulation. (Getter)"""
    function Duty()::String
        return Utils.get_string(Utils.@checked Lib.Loads_Get_duty())
    end

    """Name of the loadshape for a duty cycle simulation. (Setter)"""
    function Duty(Value::String)
        Utils.@checked Lib.Loads_Set_duty(Cstring(pointer(Value)))
    end

    """Load Index (Getter)"""
    function Idx()::Int
        return Utils.@checked Lib.Loads_Get_idx()
    end

    """Load Index (Setter)"""
    function Idx(Value::Int)
        Utils.@checked Lib.Loads_Set_idx(Value)
    end

    """Set kV rating for active Load. For 2 or more phases set Line-Line kV. Else actual kV across terminals. (Getter)"""
    function kV()::Float64
        return Utils.@checked Lib.Loads_Get_kV()
    end
    ...
    function XfkVA()::Float64
        return Utils.@checked Lib.Loads_Get_xfkVA()
    end
    ...
    function XfkVA(Value::Float64)
        Utils.@checked Lib.Loads_Set_xfkVA(Value)
    end
