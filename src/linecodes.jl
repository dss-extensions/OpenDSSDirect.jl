module LineCodes
    ...
    function Cmatrix(Value::Vector{Float64})
        ...
        Utils.@checked Lib.LineCodes_Set_Cmatrix(ValuePtr, ValueCount)
    end

    """Number of LineCodes"""
    function Count()::Int
        return Utils.@checked Lib.LineCodes_Get_Count()
    end

    """Emergency ampere rating (Getter)"""
    function EmergAmps()::Float64
        return Utils.@checked Lib.LineCodes_Get_EmergAmps()
    end

    """Emergency ampere rating (Setter)"""
    function EmergAmps(Value::Float64)
        Utils.@checked Lib.LineCodes_Set_EmergAmps(Value)
    end

    function First()::Int
        return Utils.@checked Lib.LineCodes_Get_First()
    end

    """Flag denoting whether impedance data were entered in symmetrical components"""
    function IsZ1Z0()::Bool
        return Utils.@checked(Lib.LineCodes_Get_IsZ1Z0()) != 0
    end
    ...
    function Xmatrix(Value::Vector{Float64})
        Value, ValuePtr, ValueCount = Utils.prepare_float64_array(Value)
        Utils.@checked Lib.LineCodes_Set_Xmatrix(ValuePtr, ValueCount)
    end
