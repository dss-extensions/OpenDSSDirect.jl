...
function Description(dss::DSSContext)::String
    return get_string(dss_ccall(dss.capi.Error_Get_Description, dss.ctx))
end
...
function Description(dss::DSSContext, Value::String)
    @checked dss_ccall(dss.capi.Error_Set_Description, dss.ctx, Value)
end
...
function ExtendedErrors(dss::DSSContext)::Bool
    return @checked(dss_ccall(dss.capi.Error_Get_ExtendedErrors, dss.ctx)) != 0
end
...
function ExtendedErrors(dss::DSSContext, Value::Bool)
    @checked dss_ccall(dss.capi.Error_Set_ExtendedErrors, dss.ctx, Value ? 1 : 0)
end
...
function EarlyAbort(dss::DSSContext)::Bool
    return (@checked dss_ccall(dss.capi.Error_Get_EarlyAbort, dss.ctx)) != 0
end
...
function EarlyAbort(dss::DSSContext, Value::Bool)
    @checked dss_ccall(dss.capi.Error_Set_EarlyAbort, dss.ctx, Value ? 1 : 0)
end
