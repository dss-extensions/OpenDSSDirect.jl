module Generators
    ...
    function Count()::Int
        return Utils.@checked Lib.Generators_Get_Count()
    end

    """Sets first Generator to be active.  Returns 0 if none."""
    function First()::Int
        return Utils.@checked Lib.Generators_Get_First()
    end

    """Indicates whether the generator is forced ON regardles of other dispatch criteria."""
    function ForcedON()::Bool
        return Utils.@checked(Lib.Generators_Get_ForcedON()) != 0
    end

    """Indicates whether the generator is forced ON regardles of other dispatch criteria."""
    function ForcedON(Value::Bool)
        Utils.@checked Lib.Generators_Set_ForcedON(Value ? 1 : 0)
    end

    """Generator Model"""
    function Model()::Int
        return Utils.@checked Lib.Generators_Get_Model()
    end

    """Generator Model"""
    function Model(Value::Int)
        Utils.@checked Lib.Generators_Set_Model(Value)
    end

    """Sets a generator active by name."""
    function Name()::String
        return Utils.get_string(Utils.@checked Lib.Generators_Get_Name())
    end

    """Sets a generator active by name."""
    function Name(Value::String)
        Utils.@checked Lib.Generators_Set_Name(Cstring(pointer(Value)))
    end

    """Sets next Generator to be active.  Returns 0 if no more."""
    function Next()::Int
        return Utils.@checked Lib.Generators_Get_Next()
    end

    """Power factor (pos. = producing vars). Updates kvar based on present kW value."""
    function PF()::Float64
        return Utils.@checked Lib.Generators_Get_PF()
    end

    """Power factor (pos. = producing vars). Updates kvar based on present kW value."""
    function PF(Value::Float64)
        Utils.@checked Lib.Generators_Set_PF(Value)
    end

    """Number of phases"""
    function Phases()::Int
        return Utils.@checked Lib.Generators_Get_Phases()
    end

    """Number of phases"""
    function Phases(Value::Int)
        Utils.@checked Lib.Generators_Set_Phases(Value)
    end
