...
function IsDelta(dss::DSSContext, Value::Bool)
    return @checked Lib.WindGens_Set_IsDelta(dss.ctx, Value)
end
...
function Phases(dss::DSSContext, Value::Int)
    return @checked Lib.WindGens_Set_Phases(dss.ctx, Value)
end
Phases(Value::Int) = Phases(DSS_DEFAULT_CTX, Value)

"""Class_ (Getter)"""
function Class_(dss::DSSContext)::Float64
    return @checked Lib.WindGens_Get_Class_(dss.ctx)
end
Class_() = Class_(DSS_DEFAULT_CTX)

"""Class_ (Setter)"""
function Class_(dss::DSSContext, Value::Float64)
    return @checked Lib.WindGens_Set_Class_(dss.ctx, Value)
end
Class_(Value::Float64) = Class_(DSS_DEFAULT_CTX, Value)

"""WindGen power factor. Power factor (pos. = producing vars). (Getter)"""
function PF(dss::DSSContext)::Float64
    return @checked Lib.WindGens_Get_PF(dss.ctx)
end
PF() = PF(DSS_DEFAULT_CTX)

"""WindGen power factor. Power factor (pos. = producing vars). (Setter)"""
function PF(dss::DSSContext, Value::Float64)
    return @checked Lib.WindGens_Set_PF(dss.ctx, Value)
end
PF(Value::Float64) = PF(DSS_DEFAULT_CTX, Value)

"""Non-zero values enable reactive power and voltage control in the dynamic model. (Getter)"""
function QFlag(dss::DSSContext)::Float64
    return @checked Lib.WindGens_Get_QFlag(dss.ctx)
end
QFlag() = QFlag(DSS_DEFAULT_CTX)

"""Non-zero values enable reactive power and voltage control in the dynamic model. (Setter)"""
function QFlag(dss::DSSContext, Value::Float64)
    return @checked Lib.WindGens_Set_QFlag(dss.ctx, Value)
end
QFlag(Value::Float64) = QFlag(DSS_DEFAULT_CTX, Value)

"""Name of the loadshape for daily wind speed

(API Extension) (Getter)"""
function daily(dss::DSSContext)::Float64
    return @checked Lib.WindGens_Get_daily(dss.ctx)
end
...
(API Extension) (Setter)"""
function daily(dss::DSSContext, Value::Float64)
    return @checked Lib.WindGens_Set_daily(dss.ctx, Value)
end
daily(Value::Float64) = daily(DSS_DEFAULT_CTX, Value)

"""Turbine performance coefficient. (Getter)"""
function Cp(dss::DSSContext)::Float64
    return @checked Lib.WindGens_Get_Cp(dss.ctx)
end
Cp() = Cp(DSS_DEFAULT_CTX)

"""Turbine performance coefficient. (Setter)"""
function Cp(dss::DSSContext, Value::Float64)
    return @checked Lib.WindGens_Set_Cp(dss.ctx, Value)
end
Cp(Value::Float64) = Cp(DSS_DEFAULT_CTX, Value)

"""Q control mode (0:Q, 1:PF, 2:VV). (Getter)"""
function QMode(dss::DSSContext)::Float64
    return @checked Lib.WindGens_Get_QMode(dss.ctx)
end
QMode() = QMode(DSS_DEFAULT_CTX)

"""Q control mode (0:Q, 1:PF, 2:VV). (Setter)"""
function QMode(dss::DSSContext, Value::Float64)
    return @checked Lib.WindGens_Set_QMode(dss.ctx, Value)
end
QMode(Value::Float64) = QMode(DSS_DEFAULT_CTX, Value)

"""Number of pole pairs of the induction generator (Getter)"""
function NPoles(dss::DSSContext)::Float64
    return @checked Lib.WindGens_Get_NPoles(dss.ctx)
end
NPoles() = NPoles(DSS_DEFAULT_CTX)

"""Number of pole pairs of the induction generator (Setter)"""
function NPoles(dss::DSSContext, Value::Float64)
    return @checked Lib.WindGens_Set_NPoles(dss.ctx, Value)
end
NPoles(Value::Float64) = NPoles(DSS_DEFAULT_CTX, Value)

"""Rotor radius in meters (Getter)"""
function Rad(dss::DSSContext)::Float64
    return @checked Lib.WindGens_Get_Rad(dss.ctx)
end
Rad() = Rad(DSS_DEFAULT_CTX)

"""Rotor radius in meters (Setter)"""
function Rad(dss::DSSContext, Value::Float64)
    return @checked Lib.WindGens_Set_Rad(dss.ctx, Value)
end
