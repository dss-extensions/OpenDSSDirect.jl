...
"""Set the active Storage object by name."""
...
"""Get/set the requested kvar value. Final kvar is subjected to the inverter ratings. Sets inverter to operate in constant kvar mode. (Getter)"""
function kvar(dss::DSSContext)::Float64
    return @checked Lib.Storages_Get_kvar(dss.ctx)
end
kvar() = kvar(DSS_DEFAULT_CTX)

"""Get/set the requested kvar value. Final kvar is subjected to the inverter ratings. Sets inverter to operate in constant kvar mode. (Setter)"""
function kvar(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_kvar(dss.ctx, Value)
end
kvar(Value::Float64) = kvar(DSS_DEFAULT_CTX, Value)

"""Time of day in fractional hours (0230 = 2.5) at which Storage element will automatically go into charge state. (Getter)"""
...
"""Time of day in fractional hours (0230 = 2.5) at which Storage element will automatically go into charge state. (Setter)"""
function TimeChargeTrig(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_TimeChargeTrig(dss.ctx, Value)
end
TimeChargeTrig(Value::Float64) = TimeChargeTrig(DSS_DEFAULT_CTX, Value)

"""Get/set the requested kW value. Final kW is subjected to the inverter ratings. (Getter)"""
function kW(dss::DSSContext)::Float64
    return @checked Lib.Storages_Get_kW(dss.ctx)
end
kW() = kW(DSS_DEFAULT_CTX)

"""Get/set the requested kW value. Final kW is subjected to the inverter ratings. (Setter)"""
function kW(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_kW(dss.ctx, Value)
end
kW(Value::Float64) = kW(DSS_DEFAULT_CTX, Value)

"""Tolerance (%) for the closed loop controller of the inverter (Getter)"""
function PITol(dss::DSSContext)::Float64
    return @checked Lib.Storages_Get_PITol(dss.ctx)
end
PITol() = PITol(DSS_DEFAULT_CTX)

"""Tolerance (%) for the closed loop controller of the inverter (Setter)"""
function PITol(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_PITol(dss.ctx, Value)
end
PITol(Value::Float64) = PITol(DSS_DEFAULT_CTX, Value)

"""Dispatch trigger value for charging the Storage. (Getter)"""
...
"""Dispatch trigger value for charging the Storage. (Setter)"""
function ChargeTrigger(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_ChargeTrigger(dss.ctx, Value)
end
ChargeTrigger(Value::Float64) = ChargeTrigger(DSS_DEFAULT_CTX, Value)

"""Indicates if the reactive power generation/absorption does not respect the inverter status (Getter)"""
...
"""Indicates if the reactive power generation/absorption does not respect the inverter status (Setter)"""
function VarFollowInverter(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_VarFollowInverter(dss.ctx, Value)
end
VarFollowInverter(Value::Float64) = VarFollowInverter(DSS_DEFAULT_CTX, Value)

"""Get/set the requested PF value. (Getter)"""
function PF(dss::DSSContext)::Float64
    return @checked Lib.Storages_Get_PF(dss.ctx)
end
PF() = PF(DSS_DEFAULT_CTX)

"""Get/set the requested PF value. (Setter)"""
function PF(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_PF(dss.ctx, Value)
end
PF(Value::Float64) = PF(DSS_DEFAULT_CTX, Value)

"""Percentage efficiency for CHARGING the Storage element. (Getter)"""
...
"""Percentage efficiency for CHARGING the Storage element. (Setter)"""
function EffCharge(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_EffCharge(dss.ctx, Value)
end
EffCharge(Value::Float64) = EffCharge(DSS_DEFAULT_CTX, Value)

"""Current limit per phase for the IBR when operating in GFM mode. (Getter)"""
...
"""Current limit per phase for the IBR when operating in GFM mode. (Setter)"""
function AmpLimit(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_AmpLimit(dss.ctx, Value)
end
AmpLimit(Value::Float64) = AmpLimit(DSS_DEFAULT_CTX, Value)

"""Indicates the voltage level (%) respect to the base voltage level for which the Inverter will operate. (Getter)"""
...
"""Indicates the voltage level (%) respect to the base voltage level for which the Inverter will operate. (Setter)"""
function SafeVoltage(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_SafeVoltage(dss.ctx, Value)
end
SafeVoltage(Value::Float64) = SafeVoltage(DSS_DEFAULT_CTX, Value)

"""Percentage efficiency for DISCHARGING the Storage element. (Getter)"""
...
"""Percentage efficiency for DISCHARGING the Storage element. (Setter)"""
function EffDischarge(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_EffDischarge(dss.ctx, Value)
end
...
For 3-phase, limits the positive-sequence current but not the negative-sequence." (Getter)"""
...
"""Limits current magnitude to Vminpu value for both 1-phase and 3-phase Storage similar to Generator Model 7.
For 3-phase, limits the positive-sequence current but not the negative-sequence." (Setter)"""
function LimitCurrent(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_LimitCurrent(dss.ctx, Value)
end
LimitCurrent(Value::Float64) = LimitCurrent(DSS_DEFAULT_CTX, Value)

"""Proportional gain for the PI controller within the inverter.
Use it to modify the controller response in dynamics simulation mode. (Getter)"""
...
"""Proportional gain for the PI controller within the inverter.
Use it to modify the controller response in dynamics simulation mode. (Setter)"""
function Kp(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_Kp(dss.ctx, Value)
end
Kp(Value::Float64) = Kp(DSS_DEFAULT_CTX, Value)

"""Rated Storage capacity in kWh. (Getter)"""
...
"""Rated Storage capacity in kWh. (Setter)"""
function kWhRated(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_kWhRated(dss.ctx, Value)
end
kWhRated(Value::Float64) = kWhRated(DSS_DEFAULT_CTX, Value)

"""Use it for fine tuning the current limiter when active. (Getter)"""
...
"""Use it for fine tuning the current limiter when active. (Setter)"""
function AmpLimitGain(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_AmpLimitGain(dss.ctx, Value)
end
AmpLimitGain(Value::Float64) = AmpLimitGain(DSS_DEFAULT_CTX, Value)

"""Nominal rated (1.0 per unit) voltage, kV, for Storage element. (Getter)"""
function kV(dss::DSSContext)::Float64
    return @checked Lib.Storages_Get_kV(dss.ctx)
end
kV() = kV(DSS_DEFAULT_CTX)

"""Nominal rated (1.0 per unit) voltage, kV, for Storage element. (Setter)"""
function kV(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_kV(dss.ctx, Value)
end
kV(Value::Float64) = kV(DSS_DEFAULT_CTX, Value)

"""Dispatch trigger value for discharging the Storage. (Getter)"""
...
"""Dispatch trigger value for discharging the Storage. (Setter)"""
function DischargeTrigger(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_DischargeTrigger(dss.ctx, Value)
end
DischargeTrigger(Value::Float64) = DischargeTrigger(DSS_DEFAULT_CTX, Value)

"""Inverter nameplate capability (in kVA). Used as the base for Dynamics mode and Harmonics mode values. (Getter)"""
function kVA(dss::DSSContext)::Float64
    return @checked Lib.Storages_Get_kVA(dss.ctx)
end
kVA() = kVA(DSS_DEFAULT_CTX)

"""Inverter nameplate capability (in kVA). Used as the base for Dynamics mode and Harmonics mode values. (Setter)"""
function kVA(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_kVA(dss.ctx, Value)
end
kVA(Value::Float64) = kVA(DSS_DEFAULT_CTX, Value)

"""kW rating of power output. Base for Loadshapes when DispMode=Follow. Sets kVA property if it has not been specified yet. (Getter)"""
function kWRated(dss::DSSContext)::Float64
    return @checked Lib.Storages_Get_kWRated(dss.ctx)
end
kWRated() = kWRated(DSS_DEFAULT_CTX)

"""kW rating of power output. Base for Loadshapes when DispMode=Follow. Sets kVA property if it has not been specified yet. (Setter)"""
function kWRated(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_kWRated(dss.ctx, Value)
end
kWRated(Value::Float64) = kWRated(DSS_DEFAULT_CTX, Value)

"""Control mode for the inverter. It can be one of {GFM = 1 | GFL* = 0}. (Getter)"""
...
"""Control mode for the inverter. It can be one of {GFM = 1 | GFL* = 0}. (Setter)"""
function ControlMode(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_ControlMode(dss.ctx, Value)
end
ControlMode(Value::Float64) = ControlMode(DSS_DEFAULT_CTX, Value)

"""Rated voltage (kV) at the input of the inverter while the storage is discharging (Getter)"""
function kVDC(dss::DSSContext)::Float64
    return @checked Lib.Storages_Get_kVDC(dss.ctx)
end
kVDC() = kVDC(DSS_DEFAULT_CTX)

"""Rated voltage (kV) at the input of the inverter while the storage is discharging (Setter)"""
function kVDC(dss::DSSContext, Value::Float64)
    return @checked Lib.Storages_Set_kVDC(dss.ctx, Value)
end
