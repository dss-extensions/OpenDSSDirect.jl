module CapControls
    ...
    function Reset()
        Utils.@checked Lib.CapControls_Reset()
    end
    ...
    function DelayOff(Value::Float64)
        Utils.@checked Lib.CapControls_Set_DelayOff(Value)
    end

    """Sets the first CapControl as active. Return 0 if none."""
    function First()::Int
        return Utils.@checked Lib.CapControls_Get_First()
    end

    """Type of automatic controller. (Getter)"""
    function Mode()::Lib.CapControlModes
        return Utils.@checked Lib.CapControls_Get_Mode()
    end
