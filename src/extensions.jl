...
Base.convert(::Type{GeneratorStatus}, x::Integer) = GeneratorStatus(x)
Base.convert(::Type{StorageStates}, x::Integer) = StorageStates(x)
Base.convert(::Type{DSSMessageType}, x::Integer) = DSSMessageType(x)
Base.convert(::Type{DSSSaveFlags}, x::Integer) = DSSSaveFlags(x)
...
Lib.DSSMessageType
@doc """
$( join(map(x -> "- `" * String(x[1]) * "`: " * string(x[2]), CEnum.name_value_pairs(Lib.DSSSaveFlags)), "\n") )
"""
Lib.DSSSaveFlags
